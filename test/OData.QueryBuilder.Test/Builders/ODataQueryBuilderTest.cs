...
namespace OData.QueryBuilder.Test.Builders
{
    public class ODataQueryBuilderTest
    {
        ...
        {
            var ex = Assert.Throws<ArgumentException>(() => new ODataQueryBuilder(baseUrl));
        ...
        public void ODataQueryBuilder_New_Instance_BaseUrl_Uri_Exception()
        {
            ...
            var ex = Assert.Throws<ArgumentException>(() => new ODataQueryBuilder(uri));
            ex.Message.Should().Be($"baseUrl is null");
        }

        [Theory(DisplayName = "new instance generic ODataQueryBuilder with baseUrl => Exception")]
        [InlineData("")]
        [InlineData(null)]
        public void Generic_ODataQueryBuilder_New_Instance_BaseUrl_Exception(string baseUrl)
        {
            var ex = Assert.Throws<ArgumentException>(() => new ODataQueryBuilder<ODataInfoContainer>(baseUrl));
            ex.Message.Should().Be($"{nameof(baseUrl)} is null");
        }

        [Fact(DisplayName = "new instance generic ODataQueryBuilder with baseUrl Uri => Exception")]
        public void Generic_ODataQueryBuilder_New_Instance_BaseUrl_Uri_Exception()
        {
            var uri = default(System.Uri);
