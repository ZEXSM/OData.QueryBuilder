    ...
    public class ODataQueryParameter : IODataQueryParameter
    {
        ...
        public Dictionary<string, string> ToDictionary()
        {
            var odataOperators = _queryBuilder.ToString()
                .Split(new char[2] { ODataQuerySeparators.QueryCharBegin, ODataQuerySeparators.QueryCharSeparator }, StringSplitOptions.RemoveEmptyEntries);

            var dictionary = new Dictionary<string, string>(odataOperators.Length - 1);

            for (var step = 1; step < odataOperators.Length; step++)
            {
                var odataOperator = odataOperators[step].Split(ODataQuerySeparators.QueryCharEqualSign);
        ...
        public Uri ToUri() => new Uri(_queryBuilder.ToString().TrimEnd(ODataQuerySeparators.QueryCharSeparator));
    }
}
